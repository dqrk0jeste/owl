...
static void calculate_masters_dimensions(
  ...
  *height = output_box.width - 2 * outer_gaps - 2 * border_width;
}
...
static void toplevel_update_borders_size(struct owl_toplevel *toplevel) {
  assert(toplevel->borders[0] != NULL);
...
static void toplevel_borders_set_active(struct owl_toplevel *toplevel, bool active) {
  assert(toplevel->borders[0] != NULL);

  if(active) {
    for(size_t i = 0; i < 4; i++) {
      wlr_scene_rect_set_color(toplevel->borders[i], server.config->active_border_color);
    }
  } else {
    for(size_t i = 0; i < 4; i++) {
...
/* this function should only be called on already drawn toplevels,
 * that is, when their pending state has resolved */
static void focus_toplevel(struct owl_toplevel *toplevel) {
  ...
  if(prev_toplevel != NULL) {
    ...
    if(toplevel->borders[0] != NULL) {
      toplevel_borders_set_active(prev_toplevel, false);
    }
  }
...
static void focus_output(struct owl_output *output) {
  ...
  if(output->active_workspace->fullscreen_toplevel != NULL) {
  ...
  } else if(!wl_list_empty(&output->active_workspace->masters)) {
      focus_next = wl_container_of(output->active_workspace->masters.next,
        focus_next, link);
    ...
    focus_next = wl_container_of(output->active_workspace->floating_toplevels.next,
      focus_next, link);
  }
...
static void layout_swap_tiled_toplevels(
  ...
  layout_configure_tiled_toplevels(t1->workspace);
}
...
static void toplevel_unset_fullscreen(struct owl_toplevel *toplevel) {
  ...
  layout_configure_tiled_toplevels(workspace);
}
...
static void xdg_toplevel_handle_unmap(struct wl_listener *listener, void *data) {
  ...
  if(toplevel_is_master(toplevel)) {
    ...
    /* we remove this toplevel and find a new master to replace him if possible */
...
static void xdg_toplevel_handle_commit(struct wl_listener *listener, void *data) {
    ...
    if(toplevel->floating) {
      ...
      wl_list_insert(&toplevel->workspace->masters, &toplevel->link);
...
static void layer_surface_handle_map(struct wl_listener *listener, void *data) {
  ...
  if(temp.width != output->usable_area.width || temp.height != output->usable_area.height) {
    layout_configure_tiled_toplevels(output->active_workspace);
  }
